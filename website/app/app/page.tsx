...
function resolveInitialView(startParam: string | null): View {
  if (typeof window !== "undefined") {
    const q = new URLSearchParams(window.location.search).get("view");
    if (q && VIEW_ALIASES[q]) return VIEW_ALIASES[q];
  }
  if (startParam && VIEW_ALIASES[startParam]) return VIEW_ALIASES[startParam];
  return "home";
}
...
export default function MiniApp() {
  ...
  useEffect(() => {
    ...
    setViewState(resolveInitialView(startParam));
  }, []);
  ...
  const goContact = useCallback((interest: string) => {
