...
## 90. Stage D4a：受控桌面出站桥——内嵌账号 autopilot 经闸门入队（服务端骨架）

**实施前的思考**：D1–D3 让桌面壳/扩展能内嵌多平台、消息能回流统一收件箱（入站方向已通：
`core.ingestBubble` → `/api/desktop/ingest` → store）。但**出站方向断头**：desktop 模式账号
（registry `mode="desktop"`）**无服务端 worker、不被编排器接管**，统一收件箱全自动 autopilot
要发回复时 `send_via_adapters` 找不到 adapter → 发不出去。更危险的是：若硬接发送，桌面内嵌出站
会**绕过 send-gate/kill-switch**（探查确认桌面手动 `fill-composer` 仅过关键词 guard-check，不查急停）。
D4a 解这条出站路径，且把「受控」做成不可旁路的不变式。

**关键设计（闸门内置于队列）**：新增「受控出站队列」`src/inbox/desktop_outbound.py`——
autopilot 要发 → `enqueue()` **内部先过 send-gate + kill-switch**（复用 `send_guard.send_blocked`），
通过才落 pending 命令；被拦截的命令**根本不入队**（返回 `{enqueued:False, blocked:<reason>}`）。
桌面壳/扩展轮询 `pull()` 认领（pending→claimed）→ 官方页 DOM 填入并发送（复用既有 inject
`onFillComposer({text,send})` 原语）→ `ack()` 回执。**「受控」不变式**：闸门写在 enqueue 内部，
任何命令进队列前都必过闸，无旁路。

**改动**：
- `src/inbox/desktop_outbound.py`（新）：`DesktopOutboundQueue`（SQLite **durable**——后端重启不丢
  autopilot 命令）+ `enqueue`（闸门内置，guard 可注入便于单测）/ `pull`（认领 + 超时 180s 未 ack
  自动回收防客户端崩溃卡死）/ `ack`(claimed→sent/failed) / `pending_count` / `summary`；进程单例
  `get_desktop_outbound_queue`（与 inject_health/kill_switch 同模式）。
- 路由 `unified_inbox_desktop_routes.py`：`GET /api/desktop/outbound`（轮询认领）+
  `POST /api/desktop/outbound/ack`（回执）；路由清单基线同步 2 端点。
- `main.py` autosend `_autosend_deliver`：加 desktop 感知——`desktop_bridge.enabled` 且账号
  `mode="desktop"` 时路由到队列 enqueue（经闸门）；入队成功 → `delivered_as=desktop_queued`，
  被拦截 → `{ok:False, error:blocked:...}` 让 worker 记 autosend_failed。
- `autosend_worker.py`：**修投递成功判定**——除 `ok=False` 外，`delivered=False` / `blocked` 也算
  未送达（否则 kill-switch/send-gate 拦截会被误计 total_delivered 刷指标）。
- 客户端管道：`desktop/main.js` 加 `desktop:outbound-pull/ack` IPC、`extension/background.js` 加
  `outboundPull/outboundAck` handler（两端通道对称）。
- config：`inbox.l2_autosend.desktop_bridge.enabled`（默认 **false**，双重 opt-in）。

**实施中的再优化**：
1. **闸门内置而非调用方自觉**：把 send_blocked 放进 enqueue 内部 → 受控是结构性不变式，杜绝「某条
   出站路径忘了查闸」（Stage M 反复堵的旁路问题，这次从设计上根除）。
2. **durable + 超时回收**：队列落 SQLite，重启不丢主动回复；claimed 超时自动回 pending，防桌面壳崩溃
   导致命令永久卡死。resolve-先于-deliver 的不重发语义仍由上层 worker 保证。
3. **修成功判定**：顺手修了 worker 把「被拦截」误计已送达的老问题（探查发现），并加回归断言锁定。
4. **复用既有 DOM 发送原语**：客户端不新造发送逻辑——`onFillComposer({text,send})` 已是手动「填入并
   发送」验证过的路径，D4 出站只是「服务端把命令喂给同一原语」。

**能否再优化（下一步 D4b）**：① **renderer/扩展 content 的轮询→按账号分发 webview** 这最后一跳尚未接
   （需 renderer 维护 account_id→webview 映射 + 真号现场验证 DOM 发送），本阶段只铺了 HTTP 管道；
   ② Tier1 平台 `canIngest` 仍默认关（需真号 F12 校准），入站回流才全平台打开；③ 出站队列看板（pending/
   failed 可视，复用 D1c rpa-overview 模式）；④ 手动 copilot 发送也走 guarded enqueue（统一审计）。

**回归**：`test_desktop_outbound`（新增 12：闸门拦截不入队/空文本拒/认领顺序+attempts/账号隔离/
ack/超时回收/summary）+ `test_autosend_blocked_not_delivered`（新增 3：blocked 与 delivered=False 计失败、
desktop_queued 计送达）+ `test_autosend_deliver_delay` + `test_admin_route_inventory`（清单含 2 新端点）；
`desktop npm test` **134 passed**（不回归）。全量 **6321 passed / 31 skipped / 0 failed（197s）**。

**Stage D4a 收口**：桌面内嵌账号的全自动出站从「发不出去 / 发了也绕过急停」升到「**经闸门受控入队、
durable、可回收**」——「受控 autopilot」的服务端骨架闭合，autopilot 对 desktop 账号也守 send-gate/
